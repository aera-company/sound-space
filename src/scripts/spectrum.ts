...
function draw(state:string){
 ...
 const style=getComputedStyle(canvas),accent=style.getPropertyValue('--accent').trim()||'#d5dfb6';
 ...
 for(const db of [-120,-100,-80,-60,-40,-20,0]){const pos=y(db);ctx.strokeStyle='#44453f';ctx.beginPath();ctx.moveTo(left,pos);ctx.lineTo(right,pos);ctx.stroke();ctx.fillStyle='#aaa99f';ctx.textAlign='right';ctx.fillText(String(db),left-9,pos+3);}
 const ticks=width<600?[20,100,1000,10000,20000]:[20,50,100,200,500,1000,2000,5000,10000,20000];
 for(const hz of ticks){if(hz>maxHz)continue;const pos=px(hz);ctx.strokeStyle='#34362f';ctx.beginPath();ctx.moveTo(pos,top);ctx.lineTo(pos,bottom);ctx.stroke();ctx.fillStyle='#aaa99f';ctx.textAlign=hz===20?'left':hz===maxHz?'right':'center';ctx.fillText(hz>=1000?hz/1000+'k':String(hz),pos,height-8);}
 if(values.length&&node){
  ...
  if(hold){ctx.globalAlpha=.45;stroke(aggregate(peaks),'#f0eee7',[3,3]);ctx.globalAlpha=1;}
 }
 ...
 if(hoverX!==null&&hoverX>=left&&hoverX<=right&&lastPoints.length){
  ...
  ctx.strokeStyle='#f0eee7';ctx.setLineDash([2,3]);ctx.beginPath();ctx.moveTo(hoverX,top);ctx.lineTo(hoverX,bottom);ctx.stroke();ctx.setLineDash([]);
