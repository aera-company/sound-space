...
(async()=>{
  ...
  const filmLayout=await page.locator('.project-film').evaluate(section=>{const heading=section.querySelector('.film-heading').getBoundingClientRect();const description=section.querySelector('.film-heading>p').getBoundingClientRect();const video=section.querySelector('.field-film').getBoundingClientRect();return{videoRight:video.left>heading.right,descriptionBelow:description.top>section.querySelector('#film-heading').getBoundingClientRect().bottom};});
  assert.deepEqual(filmLayout,{videoRight:true,descriptionBelow:true},'film should sit to the right with its description below the title');
