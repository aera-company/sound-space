...
(async()=>{
 ...
 for(const [name,engine] of [['chromium',chromium],['webkit',webkit]]){
  ...
  try{
   ...
   await page.locator('[data-select="68"]').click();await seek(162);
   await page.getByRole('tab',{name:'Waveform',exact:true}).click();
   assert.equal(await page.locator('[data-chart-cursor]').count(),2);assert.match(await page.locator('[data-chart-cursor="68"] text').textContent(),/2:42/);
   await page.getByRole('tab',{name:'Spectrogram',exact:true}).click();
   assert.equal(await page.locator('[data-spectro-cursor]').count(),2);assert.match(await page.locator('[data-spectro-cursor="68"]').textContent(),/2:42/);
   const plot=await page.locator('[data-spectro-plot="68"]').boundingBox();
   await page.mouse.click(plot.x+plot.width*.5,plot.y+40);await page.waitForFunction(()=>Math.abs(document.querySelector('#audio-68').currentTime-431.22)<2);
   ...
   await page.waitForFunction(()=>Math.abs(document.querySelector('#audio-68').currentTime-517.464)<3);
   await page.getByRole('tab',{name:'Loudness',exact:true}).click();assert.match(await page.locator('[data-chart-cursor="68"] text').textContent(),/8:37/);
